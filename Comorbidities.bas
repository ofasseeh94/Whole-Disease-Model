...
''Frontiers in Neurology. 2022;13:803749.DOI: https://doi.org/10.3389/fneur.2022.80374
'
...
'        'The model needs annual probability here because Characteristics_Progression converts annual probability to cycle probability using Cycle_Length.
...
Function CKD_Read_From_Table(ByVal PatientValue As Double, ByVal ScoreTable As Variant) As Double

    '====================================================================================
    '   Read one patient value against one CKD score table
    '
    ' How it works:
    '   The function loops through the rows of the selected table
    '   If the patient value falls between the row lower and upper boundaries, the function returns the value in the third column (risk score)
    '
    ' Example:
    '   Patient BMI = 31
    '   CKD_BMI_Score_Table:
    '       0 to 24.999  -> 0 points
    '       25 to 200    -> 1 point
    '
    '   CKD_Read_From_Table(31, CKD_BMI_Score_Table) returns 1
    '====================================================================================

    Dim i As Long

    For i = LBound(ScoreTable) To UBound(ScoreTable)

        If PatientValue >= ScoreTable(i)(0) And PatientValue <= ScoreTable(i)(1) Then
            CKD_Read_From_Table = ScoreTable(i)(2)
            Exit Function
        End If
